...
const Page = () => {
    ...
    const { data: products = [], isLoading } = trpc.products.useQuery()
    ...
    const handleEditProduct = (data: Partial<Product>) => {
        ...
        // Ensure all required fields are present before passing to mutation
        const productData = {
            id: data.id,
            name: data.name || '',
    ...
    const filteredProducts = products.filter(product => {
        ...
        const matchesSearch = product.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
            (product.id?.toLowerCase().includes(searchQuery.toLowerCase()) ?? false) ||
            product.category.toLowerCase().includes(searchQuery.toLowerCase()) ||
            (product.ingredients?.some(i => i.toLowerCase().includes(searchQuery.toLowerCase())) ?? false)
                            ...
                            {isLoading ? (
                                <div className="text-center py-12">
                                    <div className="inline-flex items-center gap-2 text-gray-500">
                                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-pry"></div>
                                        Loading products...
                                    </div>
                                </div>
                            ) : (
